...
pub mod token {
    ...
    pub trait TokenInterface {
        ...
        // Transfer the balance from owner's account to contract account
    ...
    impl TokenInterface for TokenContract {
        ...
        fn transferFrom(&mut self, from: Address, to: Address, amount: U256) -> bool {
            ...
            let recipientBalance = read_balance_of(&to);
            if  amount == 0.into() || fromBalance < amount {
                false
            } else {
                let new_from_balance = fromBalance - amount;
                let new_recipient_balance = recipientBalance + amount;
                pwasm_ethereum::write(&balance_key(&from), &new_from_balance.into());
                pwasm_ethereum::write(&balance_key(&to), &new_recipient_balance.into());
